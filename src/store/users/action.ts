...
export const getUsersCollectionAction = createAsyncThunk(
  ...
  async (arg: string, { getState }) => {
    try {
      ...
      const res = await fetch(
        `/api/user-exists?email=${session?.user?.email}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
...
export const changePassword = createAsyncThunk(
  ...
  async (
    args: { _id: string; old_password: string; confirm_password: string },
    { getState }
  ) => {
    try {
      const session = (await getSession()) as Session;
