...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return [
      {
        // 1. The path your frontend will call (e.g., fetch('/api/external/pincode/110001'))
        source: '/api/external/:path*',

        // 2. The actual external API URL to which the request should be forwarded
        destination: 'https://api.postalpincode.in/:path*',
      },
    ];
  },
  eslint: {
